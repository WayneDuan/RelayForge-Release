...
function Workspace() {
  ...
  const removeNode = async (node: ApiItem) => {
    if (!isAdmin) return;
    const tunnelCount = tunnels.filter((tunnel) => Number(tunnel.inNodeId) === Number(node.id) || Number(tunnel.outNodeId) === Number(node.id)).length;
    if (tunnelCount > 0) {
      toast.error(`该节点仍被 ${tunnelCount} 条隧道使用，请先删除或迁移隧道`);
      return;
    }
    if (!window.confirm(`确定删除节点“${node.name || node.id}”吗？`)) return;
    const response = await deleteNode(Number(node.id));
    if (response.code !== 0) {
      toast.error(response.msg || "节点删除失败");
      return;
    }
    setNodes((current) => current.filter((item) => Number(item.id) !== Number(node.id)));
    setNodeStats((current) => {
      const next = { ...current };
      delete next[String(node.id)];
      return next;
    });
    recordActivity("删除节点", node.name || `Node-${node.id}`, "amber");
    toast.success("节点已删除");
  };
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        <div className="content-area">
          ...
          {activeView === "nodes" && isAdmin && <NodesView nodes={nodes} tunnels={tunnels} forwards={forwards} nodeStats={nodeStats} onCreate={() => setCreator("nodes")} onEdit={setSelectedNode} onDelete={removeNode} onRefresh={refreshNode} onInstall={installNode} onReload={() => void loadData()} />}
...
function NodesView({ nodes, tunnels, forwards, nodeStats, onCreate, onEdit, onDelete, onRefresh, onInstall, onReload }: any) {
