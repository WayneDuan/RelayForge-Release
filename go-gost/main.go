...
func main() {
	...
	if err := os.Chdir(filepath.Dir(configPath)); err != nil {
		fmt.Printf("❌ 无法切换到配置目录: %v\n", err)
		os.Exit(1)
	}
	config, err := LoadConfig(configPath)
	if err != nil {
		fmt.Printf("❌ 配置加载失败: %v\n", err)
		fmt.Printf("请确保配置文件存在: %s\n", configPath)
		os.Exit(1)
	}
	...
	wsReporter := socket.StartWebSocketReporterWithConfig(config.Addr, config.Secret, config.Http, config.Tls, config.Socks, AgentVersion)
	defer wsReporter.Stop()
	service.SetHTTPReportURL(config.Addr, config.Secret)

	updaterContext, stopUpdater := context.WithCancel(context.Background())
	defer stopUpdater()
	go runAutoUpdater(updaterContext, config)
