...
// TcpProbeListenerRequest starts or stops a short-lived listener used only for link diagnostics.
type TcpProbeListenerRequest struct {
	Port int `json:"port"`
}

type tcpProbeListener struct {
	listener   net.Listener
	references int
}

// HttpProbeRequest HTTP/HTTPS 网站探测请求
...
// HttpProbeResponse HTTP/HTTPS 网站探测响应
type HttpProbeResponse struct {
	URL           string  `json:"url"`
	FinalURL      string  `json:"finalUrl,omitempty"`
	StatusCode    int     `json:"statusCode,omitempty"`
	Success       bool    `json:"success"`   // 2xx/3xx
	Reachable     bool    `json:"reachable"` // 已收到 HTTP 响应，包括 4xx/5xx
	Redirected    bool    `json:"redirected,omitempty"`
	Duration      float64 `json:"duration"` // 请求耗时(ms)
...
type WebSocketReporter struct {
	...
	probeListeners map[int]*tcpProbeListener
}
...
func NewWebSocketReporter(serverURL string, secret string) *WebSocketReporter {
	...
	return &WebSocketReporter{
		...
		connecting:     false,
		aesCrypto:      aesCrypto,
		probeListeners: make(map[int]*tcpProbeListener),
	}
}
...
func (w *WebSocketReporter) connect() error {
	...
	headers := http.Header{}
	headers.Set("X-RelayForge-Node-Secret", w.secret)
...
func (w *WebSocketReporter) routeCommand(cmd CommandMessage) {
	...
	case "StartTcpProbeListener":
		err = w.startTcpProbeListener(cmd.Data)
		response.Type = "StartTcpProbeListenerResponse"

	case "StopTcpProbeListener":
		err = w.stopTcpProbeListener(cmd.Data)
		response.Type = "StopTcpProbeListenerResponse"

	// HTTP/HTTPS 网站诊断命令
	case "HttpProbe":
		var httpProbeResult HttpProbeResponse
		httpProbeResult, err = w.handleHttpProbe(cmd.Data)
		response.Type = "HttpProbeResponse"
		response.Data = httpProbeResult
...
func (w *WebSocketReporter) handleUpdateLimiter(data interface{}) error {
	...
	if updateReq.Limiter == "" {
		updateReq.Limiter = updateReq.Data.Name
	}
...
func buildWebSocketURL(addr, secret, version string, httpValue, tlsValue, socksValue int) (string, error) {
	rawAddr := strings.TrimSpace(addr)
	if !strings.HasPrefix(rawAddr, "ws://") && !strings.HasPrefix(rawAddr, "wss://") {
		rawAddr = "wss://" + rawAddr
	}
	u, err := url.Parse(rawAddr)
	if err != nil || u.Host == "" || (u.Scheme != "ws" && u.Scheme != "wss") {
		if err == nil {
			err = fmt.Errorf("地址必须包含有效的 ws:// 或 wss:// 主机")
		}
		return "", err
	}
	...
	query.Set("http", strconv.Itoa(httpValue))
	query.Set("tls", strconv.Itoa(tlsValue))
	query.Set("socks", strconv.Itoa(socksValue))
...
func (w *WebSocketReporter) startTcpProbeListener(data interface{}) error {
	var request TcpProbeListenerRequest
	jsonData, err := json.Marshal(data)
	if err != nil {
		return fmt.Errorf("序列化诊断监听请求失败: %v", err)
	}
	if err := json.Unmarshal(jsonData, &request); err != nil {
		return fmt.Errorf("解析诊断监听请求失败: %v", err)
	}
	if request.Port <= 0 || request.Port > 65535 {
		return fmt.Errorf("无效的诊断端口")
	}

	w.probeMutex.Lock()
	defer w.probeMutex.Unlock()
	if current := w.probeListeners[request.Port]; current != nil {
		current.references++
		return nil
	}

	listener, err := net.Listen("tcp", net.JoinHostPort("", strconv.Itoa(request.Port)))
	if err != nil {
		return fmt.Errorf("启动诊断监听失败: %v", err)
	}
	w.probeListeners[request.Port] = &tcpProbeListener{listener: listener, references: 1}
	go serveTcpProbeListener(listener)
	go w.expireTcpProbeListener(request.Port, listener)
	return nil
}

func (w *WebSocketReporter) stopTcpProbeListener(data interface{}) error {
	var request TcpProbeListenerRequest
	jsonData, err := json.Marshal(data)
	if err != nil {
		return fmt.Errorf("序列化诊断监听请求失败: %v", err)
	...
	w.probeMutex.Lock()
	defer w.probeMutex.Unlock()
	current := w.probeListeners[request.Port]
	if current == nil {
		return nil
	}
	current.references--
	...
	delete(w.probeListeners, request.Port)
	return current.listener.Close()
}

func (w *WebSocketReporter) closeTcpProbeListeners() {
	w.probeMutex.Lock()
	defer w.probeMutex.Unlock()
	for port, current := range w.probeListeners {
		current.listener.Close()
		delete(w.probeListeners, port)
	}
}

func serveTcpProbeListener(listener net.Listener) {
	for {
...
func (w *WebSocketReporter) expireTcpProbeListener(port int, listener net.Listener) {
	time.Sleep(30 * time.Second)
	w.probeMutex.Lock()
	defer w.probeMutex.Unlock()
	current := w.probeListeners[port]
	if current == nil || current.listener != listener {
		return
	}
	delete(w.probeListeners, port)
	listener.Close()
...
// handleHttpProbe 从节点执行一次真实的 HTTP GET 请求。
func (w *WebSocketReporter) handleHttpProbe(data interface{}) (HttpProbeResponse, error) {
	jsonData, err := json.Marshal(data)
	if err != nil {
		return HttpProbeResponse{}, fmt.Errorf("序列化HTTP探测数据失败: %v", err)
	}

	var req HttpProbeRequest
	if err := json.Unmarshal(jsonData, &req); err != nil {
		return HttpProbeResponse{}, fmt.Errorf("解析HTTP探测请求失败: %v", err)
	}

	req.URL = strings.TrimSpace(req.URL)
	response := HttpProbeResponse{URL: req.URL, RequestId: req.RequestId}
	parsedURL, err := url.Parse(req.URL)
	if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
		response.ErrorType = "invalid_url"
		response.ErrorMessage = "URL必须是有效的http://或https://地址"
		return response, nil
	}
	...
	ctx, cancel := context.WithTimeout(w.ctx, time.Duration(req.Timeout)*time.Millisecond)
	defer cancel()
	httpRequest, err := http.NewRequestWithContext(ctx, http.MethodGet, req.URL, nil)
	if err != nil {
		response.ErrorType = "invalid_url"
		response.ErrorMessage = err.Error()
		return response, nil
	}
	httpRequest.Header.Set("User-Agent", "RelayForge-Node-Diagnostic/1.0")
	httpRequest.Header.Set("Accept", "text/html,application/xhtml+xml,application/json;q=0.9,*/*;q=0.8")

	// 诊断应反映节点自身的直连能力，不通过宿主机环境中的 HTTP 代理。
	...
	response.Duration = time.Since(start).Seconds() * 1000
	if err != nil {
		response.ErrorType = classifyHTTPProbeError(err)
		response.ErrorMessage = err.Error()
		return response, nil
	}
	...
	response.Redirected = response.FinalURL != response.URL
	...
	response.ErrorType = "http_status"
	response.ErrorMessage = fmt.Sprintf("网站已响应，但 HTTP 状态为 %d", resp.StatusCode)
	return response, nil
}

func classifyHTTPProbeError(err error) string {
