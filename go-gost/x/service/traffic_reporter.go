...
func SetHTTPReportURL(addr string, secret string) {
	...
	httpReportURL = strings.TrimRight(baseURL, "/") + "/flow/upload"
	configReportURL = strings.TrimRight(baseURL, "/") + "/flow/config"
	reportSecret = secret
...
func sendTrafficReport(ctx context.Context, reportItems TrafficReportItem) (bool, error) {
	...
	req.Header.Set("X-RelayForge-Node-Secret", reportSecret)
